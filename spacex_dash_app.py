...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',
                                    ...
                                    searchable = 'True'),
                                html.Br(),
                                ## JQW: Dropdown implementation Successful
...
def pie(site_dropdown):
    if site_dropdown == 'All': 
        title_pie = f'Launch Result for All Launch Sites'
        fig = px.pie(spacex_df,values='class',names='Launch Site',title=title_pie)
    else:
        ddFilter = spacex_df[spacex_df['Launch Site'] == site_dropdown]
        gbFilter = ddFilter.groupby(['Launch Site','class']).size().reset_index(name = 'class count')
        title_pie = f'Launch Result for {site_dropdown} Launch Site'
        fig = px.pie(gbFilter,values='class count',names = 'class',title=title_pie)
    return fig
...
def build_scatter(site,payload):
    ...
    filtered_dfa = spacex_df[spacex_df['Payload Mass (kg)'].between(low,high)]
